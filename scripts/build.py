...
def build_pyinstaller(onefile: bool = True) -> Path:
    ...
    # Entry point
    entry_point = SRC_DIR / "fabric" / "main.py"
    ...
    if onefile:
        executable = DIST_DIR / f"fabric-{get_version()}{ext}"
    else:
        executable = DIST_DIR / f"fabric-{get_version()}" / f"fabric-{get_version()}{ext}"
...
"""
        ...
        release_dir = DIST_DIR / f"fabric-{get_version()}-linux"
        release_dir.mkdir(exist_ok=True)

        # Copy files
        shutil.copy(executable, release_dir / "fabric")
        ...
        # Create tarball
        tarball = DIST_DIR / f"fabric-{get_version()}-linux.tar.gz"
        subprocess.run(
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Build fabric executable")
    parser.add_argument(
        "--backend",
    ...
    version = get_version()
    print(f"Building fabric v{version}")
