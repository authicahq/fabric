...
from fabric.core.exceptions import FabricError
...
class TestCLIFunctionsDirectly:
    ...
    def test_get_backends_empty_config(self) -> None:
        ...
        from fabric.main import get_backends
    ...
    def test_version_callback_true(self) -> None:
        ...
        from fabric.main import version_callback
...
class TestMainCallback:
    ...
    @patch("fabric.main.setup_logging")
...
class TestSyncCommandDirectly:
    ...
    @patch("fabric.main.ConfigLoader")
    @patch("fabric.main.get_backends")
...
class TestConfigCommandDirectly:
    ...
    def test_config_show_displays_current(
        ...
        config(generate=False, output=Path("fabric.yaml"))
...
class TestDiscoverCommandDirectly:
    ...
    @patch("fabric.main.BackendDiscovery")
    @patch("fabric.main.console")
    def test_discover_no_backends_shows_message(
        ...
        from fabric.main import discover

        mock_discovery.return_value.discover_all.return_value = []

        discover(generate_config=False, output=Path("fabric.yaml"))
...
class TestMainErrorHandling:
    ...
    def test_sync_gguf_error(self, mock_loader: MagicMock) -> None:
        """Test sync handles FabricError."""
        import typer

        from fabric.main import sync

        mock_loader.return_value.load.side_effect = FabricError("Test error")
