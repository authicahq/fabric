...
CONFIG_LOCATIONS = [
    Path.cwd() / "fabric.yaml",
    Path.cwd() / "fabric.yml",
    Path.home() / ".config" / "fabric" / "config.yaml",
    Path.home() / ".fabric.yaml",
    Path("/etc") / "fabric" / "config.yaml",
    Path("/etc") / "default" / "fabric",
]
...
def get_env_config() -> dict[str, Any]:
    ...
    Environment variables are expected to be prefixed with FABRIC_
    and use double underscore as nested key separator.

    Examples:
        FABRIC_SOURCE_DIR=/models
        FABRIC_BACKENDS__LLAMA_CPP__OUTPUT_DIR=/llama_models
        FABRIC_WATCH__ENABLED=true
