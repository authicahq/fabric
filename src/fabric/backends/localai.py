...
class LocalAIBackend(Backend):
    ...
    def _yaml_needs_update(self, yaml_path: Path, group: ModelGroup) -> bool:
        ...
        if not yaml_path.exists():
            self.logger.debug(
                "LocalAI config doesn't exist, will generate",
                model=group.model_id,
                path=str(yaml_path),
            )
            return True

        # Load existing config to log debug info
        existing = self._load_existing_config(yaml_path, "yaml")
        self.logger.debug(
            "LocalAI config resolution",
            model=group.model_id,
            existing=True,
            context_size=existing.get("context_size") if existing else None,
            gpu_layers=existing.get("gpu_layers") if existing else None,
        )
