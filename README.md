...
- **Per-Backend Ignore Files**: Exclude specific models from specific backends using `.fabricignore`
...
With [uv](https://docs.astral.sh/uv/) installed, you can run `fabric` directly without installing:
...
Download pre-built binaries from the [releases page](https://github.com/authicahq/fabric/releases).
...
This creates a `fabric.yaml` file with default settings.
...
fabric sync --source /models --llama-cpp /llama_models --localai /localai_models
```
...
1. Configuration file (`fabric.yaml`)
...
Environment variables use the prefix `FABRIC_` with double underscores for nesting:

```bash
export FABRIC_SOURCE_DIR=/models
export FABRIC_BACKENDS__LLAMA_CPP__OUTPUT_DIR=/llama_models
export FABRIC_WATCH__ENABLED=true
```
...
```bash
fabric sync --add-only
```

Or via environment variable:

```bash
export FABRIC_SYNC__ADD_ONLY=true
```
...
Place a `.fabricignore` file in any backend's output directory:

```bash
# In /ollama_models/.fabricignore
model-a
model-b-*
